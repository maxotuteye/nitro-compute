...
def iInstructions(opcode,registers):
    ...
    if(opcode=="MOVR"):
        ...
        word+="0"
        word+="\t"
    #Exchange instruction swaps the content of 2 registers
    #by making use of a temporary register
    elif opcode=="XCHNG":
        word+=Opcode["MOVR"]+registersA[registers[0]]+registersA["R15"]+"0\t"
        word+=Opcode["MOVR"]+registersA[registers[1]]+registersA[registers[0]]+"0\t"
        word+=Opcode["MOVR"]+registersA["R15"]+registersA[registers[1]]+"0\t"
        instructionNum+=3
        output(word)
        return
    else:
